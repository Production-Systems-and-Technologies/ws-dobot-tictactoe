...
from dobot_python.message import Message

if not hasattr(serial, "Serial"):
    raise RuntimeError(
        "Real Dobot mode requires pyserial. Run without hardware with "
        "`python tictactoe.py --mock`, or install pyserial for hardware mode."
    )
...
class Interface:
    ...
    # TODO: Does not work - but is implemented according to spec. Bad documentation? Maybe fixed
    def set_jog_joint_params(self, velocity, acceleration, queue=False):
        payload = velocity + acceleration
        req = Message([0xAA,0xAA], 2, 70, True, queue, payload, 'out')
